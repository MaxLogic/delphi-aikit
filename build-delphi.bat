...
:usage_fail
echo.
echo Usage: %~nx0 ^<project.dproj^> [-ver N] [-config Debug^|Release] [-platform Platform] [-ignore-warnings List] [-ignore-hints List] [-exclude-path-masks List] [-keep-logs] [-ai] [-show-warnings] [-show-hints] [-show-warnings-on-success] [-no-brand]
goto cleanup
...
setlocal DisableDelayedExpansion
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  "$max=50; $root=[IO.Path]::GetFullPath('%ROOT%')+'\'; $rx=[regex]::Escape($root); Get-Content -LiteralPath '%PFILE%' | ForEach-Object { $s=$_; if($s -match ':\s+error ' -or $s -match ':\s+fatal '){ $s=$s -replace ('(?i)'+$rx),''; $s=$s -replace ('(?i)\['+$rx),'['; $s=[regex]::Replace($s, '^[A-Za-z]:\\(?:[^\\]+\\)*([^\\]+\([^\\)]*\):)', '$1'); $s } } | Select-Object -First $max"
endlocal & exit /b 0
