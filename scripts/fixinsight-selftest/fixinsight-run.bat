...
rem Output root for DAK outputs (summary.md, run.log, fixinsight/)
set "ML_OUT_REL=Out"
...
rem Optional: generate raw FixInsightCL outputs under Reports\fixinsight\
...
echo.
echo ============================================================
echo  FixInsight run via DelphiAIKit analyze
echo ============================================================
...
"%ML_RESOLVER_EXE%" ^
  ...
  --out "%ML_SAMPLE_DIR%" ^
  --fi-formats all ^
  --pascal-analyzer false ^
  --clean true ^
  --write-summary false ^
  %ML_OPT_ARGS% >nul 2>&1
if errorlevel 1 (
  echo [ERROR] Failed to generate sample reports.
  exit /b 10
)

echo [DONE] Raw reports saved under:
echo        "%ML_SAMPLE_DIR%\fixinsight"
