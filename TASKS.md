...
### T-125 [CLI] Reverify external Delphi LSP on Delphi 13.x once installed
Outcome:
- The capability probe is rerun against the locally installed Delphi 13.x `DelphiLSP.exe`.
...
Deps: local Delphi 13.x installation
Verify: manual, cli-proof
Notes: Add the results back into `.agents/plans/lsp.md` as soon as Delphi 13.x is available locally.
...
Outcome:
- Extract the remaining public build summary concerns from `Dak.Build` into dedicated units such as `Dak.Build.Summary` and, if justified, `Dak.Build.Types`, so `Dak.Build` becomes a thin facade over public API entry points.
- Keep the current public behavior and signatures for `TryRunBuild`, `ParseBuildLogs`, `BuildSummaryAsJson`, and `IBuildProcessRunner` unchanged while reducing `Dak.Build` size and responsibility.
- Preserve the current build test coverage and command behavior after the split.
Proof:
- Run: `cmd.exe /C build-delphi.bat tests\DelphiAIKit.Tests.dproj -config Release -platform Win32 -ver 23 -test-output-dir tests\temp\test-bin`
  Expect: Exit code `0`.
- Run: `cmd.exe /C "set DAK_TEST_RESOLVER_EXE=F:\projects\MaxLogic\DelphiAiKit\projects\tests\temp\resolver-bin\DelphiAIKit.exe && F:\projects\MaxLogic\DelphiAiKit\tests\tests\temp\test-bin\DelphiAIKit.Tests.exe --consolemode:quiet --run=Test.Build"`
  Expect: Build-focused tests pass with `Failed 0` and `Errored 0`.
- Run: `cmd.exe /C build-delphi.bat projects\DelphiAIKit.dproj -config Release -platform Win32 -ver 23 -test-output-dir tests\temp\resolver-bin`
  Expect: Exit code `0`.
...
Outcome:
- `dfm-check` no longer raises `EIntOverflow` for the reported `ToDoMax.dproj` Win32 `--all --verbose` run.
- The same command exits cleanly and keeps normal `dfm-check` output for that project.
Proof:
- Run: `F:\projects\MaxLogic\DelphiAiKit\bin\DelphiAIKit.exe dfm-check --dproj F:\projects\MaxLogic\ToDoApp\Project\ToDoMax.dproj --delphi 23.0 --config Release --platform Win32 --all --verbose`
  Expect: Exit code `0`; output does not contain `Unhandled exception: EIntOverflow`.
Touches: src/dak.dfmcheck.pas, tests/units/test.dfmcheck.pas
Verify: cli-proof, unit-test
Notes: Follow-up to the reported `Unhandled exception: EIntOverflow: Integer overflow` crash. Added a regression test for high native build exit codes so `dfm-check` now formats unsigned/hex exit codes without overflowing the CLI result path.
